...
from rapprentice.tps import tps_fit, tps_cost
from rapprentice.registration import tps_rpm_bij, unit_boxify, tps_reg_cost
...
class SelectionStrategy(metaclass=SelectionStrategyMeta):
    ...
    def select_source_demo(
        ...
        Args:
            ...
            object_nodal_positions (torch.Tensor): current object nodal positions, for the object in this subtask
...
class RegistrationCostStrategy(SelectionStrategy):
    """
    Pick source demonstration based on TPS registration cost between current object pose
    and source object poses. Lower registration cost indicates better alignment.
    """
    ...
    def select_source_demo(
        ...
        """
        Selects source demonstration index using TPS registration cost between current
        object pose and source object poses.
        ...
        Args:
            ...
            src_subtask_datagen_infos (list): DatagenInfo instance for the relevant subtask segment
                in the source demonstrations
            bend_coef (float): bending coefficient for TPS regularization
            rot_reg (float): rotation regularization coefficient for TPS
            n_iter (int): number of iterations for TPS-RPM
            nn_k (int): pick source demo index uniformly at randomly from the top @nn_k lowest cost
            use_tps_rpm (bool): whether to use TPS-RPM-BIJ for more accurate cost computation

        Returns:
            source_demo_ind (int): index of source demonstration - indicates which source subtask segment to use
        """

        # collect nodal positions from start of subtask source segments
        src_object_nodal_positions = []
        for di in src_subtask_datagen_infos:
            # use object nodal positions at start of subtask segment
            src_obj_nodal_pos = list(di.object_nodal_positions.values())
            assert len(src_obj_nodal_pos) == 1
            src_object_nodal_positions.append(src_obj_nodal_pos[0][0])

        src_object_nodal_positions = torch.stack(src_object_nodal_positions)

        # Convert to numpy for rapprentice functions
        current_nodal_pos = object_nodal_positions.cpu().numpy()

        # Compute registration costs for each source demonstration
        registration_costs = []

        for i, src_nodal_pos in enumerate(src_object_nodal_positions):
            src_nodal_pos_np = src_nodal_pos.cpu().numpy()

            if use_tps_rpm:
                # Use TPS-RPM-BIJ with unit_boxify for more accurate cost computation
                try:
                    # Scale both point clouds to unit box for better registration
                    scaled_src, _ = unit_boxify(src_nodal_pos_np)
                    scaled_current, _ = unit_boxify(current_nodal_pos)

                    # Fit TPS transformation from source to current
                    f, g = tps_rpm_bij(
                        scaled_src, 
                        scaled_current, 
                        rot_reg=rot_reg, 
                    ...
                    # Use the sum of forward and backward transformation costs
                    cost = tps_reg_cost(f) + tps_reg_cost(g)
                except Exception:
                    # Fallback to simple TPS cost if TPS-RPM fails
                    cost = float('inf')
            else:
                # Use simple TPS fitting and cost computation
                try:
                    # Fit TPS transformation from source to current
                    ...
                    cost = tps_cost(
                        lin_ag, trans_g, w_ng, 
                        src_nodal_pos_np, current_nodal_pos, 
                        bend_coef
                    )
                except Exception:
                    # Fallback cost if TPS fitting fails
                    cost = float('inf')

            registration_costs.append(cost)

        # Convert to tensor for sorting
        costs_tensor = torch.tensor(registration_costs, dtype=torch.float32)

        # Find the demonstrations with lowest registration costs
        nn_k = min(nn_k, len(costs_tensor))

        # return one of the top-K nearest neighbors uniformly at random
        rand_k = torch.randint(0, nn_k, (1,)).item()
        top_k_neighbors_in_order = torch.argsort(costs_tensor)[:nn_k]
        return top_k_neighbors_in_order[rand_k]
