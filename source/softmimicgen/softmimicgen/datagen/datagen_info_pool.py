...
class DataGenInfoPool:
    ...
    def _add_episode(self, episode: EpisodeData):
        ...
        if "datagen_info" in ep_grp["obs"]:
            ...
            # Extract rigid object poses if available
            object_poses_dict = None
            if "object_pose" in ep_grp["obs"]["datagen_info"]:
                object_poses_dict = ep_grp["obs"]["datagen_info"]["object_pose"]

            # Extract object nodal positions if available
            object_nodal_positions_dict = None
            if "object_nodal_position" in ep_grp["obs"]["datagen_info"]:
                object_nodal_positions_dict = ep_grp["obs"]["datagen_info"]["object_nodal_position"]
