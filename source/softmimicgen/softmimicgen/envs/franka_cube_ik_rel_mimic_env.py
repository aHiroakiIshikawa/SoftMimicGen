...
from softmimicgen.mdp.observations import usd_mesh_points_w


class FrankaCubeMimicEnv(ManagerBasedRLMimicEnv):
    """
    ...
    def get_object_nodal_positions(self, env_ids: Sequence[int] | None = None):
        """Gets surface mesh points for each rigid object using USD mesh vertices.

        Overrides the parent deformable-only implementation to support rigid bodies.
        Reads mesh vertices from USD, then transforms by root pose each step.
        """
        if env_ids is None:
            env_ids = slice(None)
        ...
        for obj_name in rigid_object_states:
            obj_cfg = SceneEntityCfg(obj_name)
            world_pts = usd_mesh_points_w(self, obj_cfg)
            world_pts = world_pts - self.scene.env_origins.unsqueeze(1)
            object_nodal_positions[obj_name] = world_pts[env_ids]
