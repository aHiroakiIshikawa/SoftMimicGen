...
"""
Stream a physical SO-101 leader arm's joint positions over UDP to a remote
...
Usage (via an SSH tunnel, so no EC2 security group port needs to be opened):
...
parser = argparse.ArgumentParser(description="Stream SO-101 leader arm joint positions over UDP.")
parser.add_argument(
    "--host",
    type=str,
    required=True,
...
parser.add_argument("--port", type=int, default=9999, help="UDP port of the receiving process.")
parser.add_argument(
    "--teleop_port",
    type=str,
...
def main() -> None:
    ...
    dest = (args_cli.host, args_cli.port)
    print(f"[INFO] Streaming leader joint positions to udp://{args_cli.host}:{args_cli.port}")
